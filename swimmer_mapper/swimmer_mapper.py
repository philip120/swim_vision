...
class SwimmerMapper:
    ...
    def smooth_position(self, track_id: int, current_pos: Tuple[float, float]) -> Tuple[float, float]:
        """
        Smooth the current position using historical data.
        Uses exponential moving average for x coordinate and keeps y (lane) constant.
        
        Args:
            track_id: ID of the tracked swimmer
            current_pos: Current position (x, y) on minimap
        ...
        if len(history) > self.smoothing_window:
            history = history[-self.smoothing_window:]
            self.position_history[track_id] = history
            
        if len(history) < 2:
            return current_pos
            
        # Apply exponential moving average to x coordinate
        x_smooth = current_pos[0]
        for prev_pos in reversed(history[:-1]):  # Exclude current position
            x_smooth = self.alpha * prev_pos[0] + (1 - self.alpha) * x_smooth
            
        # Keep y coordinate (lane) constant
        return (x_smooth, current_pos[1])
        
    def clean_history(self, active_tracks: List[int]) -> None:
        """
        ...
        tracked_ids = list(self.position_history.keys())
    ...
    def map_swimmers(self, swimmer_positions: Dict[int, Tuple[float, float]], 
        ...
        points are not visible. Applies smoothing to the positions.
        ...
        # Clean up history for inactive tracks
        self.clean_history(list(swimmer_positions.keys()))
        ...
        # Apply smoothing to mapped positions
        smoothed_positions = {}
        for track_id, pos in mapped_positions.items():
            smoothed_positions[track_id] = self.smooth_position(track_id, pos)
